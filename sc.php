...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['toggle_hide'])) {
    if (!isAdmin()) { logUnauthorized('越权尝试隐藏/取消隐藏文章', true); header('Location: sc.php?error=1'); exit; }
    $hideFile = basename($_POST['toggle_hide']);
    $hidePath = $dataDir . '/' . $hideFile;
    if (file_exists($hidePath) && strtolower(pathinfo($hideFile, PATHINFO_EXTENSION)) === 'md') {
        $raw = file_get_contents($hidePath);
        $meta = [];
        $content = $raw;
        if (preg_match('/<!--META(.*?)-->/s', $raw, $m)) {
            $meta = json_decode(trim($m[1]), true) ?: [];
            $content = preg_replace('/<!--META.*?-->/s', '', $raw, 1);
        }
        $hidePassword = trim($_POST['hide_password'] ?? '');
        if (!empty($meta['hidden']) && empty($hidePassword)) {
            
            unset($meta['hidden'], $meta['password_hash']);
        } else if (!empty($hidePassword)) {
            
            $meta['hidden'] = true;
            $meta['password_hash'] = password_hash($hidePassword, PASSWORD_DEFAULT);
        } else {
            header('Location: sc.php');
            exit;
        }
        $newMeta = '<!--META' . json_encode($meta, JSON_UNESCAPED_UNICODE) . '-->';
        file_put_contents($hidePath, $newMeta . $content);
    }
    header('Location: sc.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($ext === 'zip') {
            ...
            if ($zip->open($uploadedFile['tmp_name']) === true) {
                ...
                for ($i = 0; $i < $zip->numFiles; $i++) {
                    ...
                    if ($safeName === '.' || $safeName === '..') $safeName = 'doc_' . time() . '_' . $i;
    ...
    if (!$isZip && !empty($url) && empty($content)) {
        ...
        if (filter_var($urlHost, FILTER_VALIDATE_IP)) {
            $ipLong = ip2long($urlHost);
            if ($ipLong === false || ($ipLong & 0xFF000000) === 0x7F000000
                || ($ipLong & 0xFFFF0000) === 0xA9FE0000
        ...
        if (!$ssrfBlocked && !isAllowedFetchUrl($url)) {
            $ssrfBlocked = true;
        }
        if ($ssrfBlocked) {
            $uploadError = 'URL not allowed';
        } else {
            $ctx = stream_context_create(['http' => ['timeout' => 10, 'follow_location' => false, 'max_filesize' => 5242880], 'https' => ['timeout' => 10, 'verify_peer' => true]]);
...
if ($files) {
    ...
    foreach ($files as $file) {
        ...
        $fileList[] = ['name' => $filename, 'displayName' => $displayName, 'pinned' => $isPinned, 'hidden' => $isHidden];
    }
}
...
.hide-btn.is-hidden svg{fill:none;stroke:var(--accent)}
.hide-btn.is-hidden{border-color:var(--accent);color:var(--accent);background:var(--accent-light)}
.hidden-badge{display:inline-flex;align-items:center;justify-content:center;margin-right:3px;color:var(--accent);vertical-align:middle;position:relative;top:-1px}
...
.toggle-label{font-size:.9em;font-weight:500}
        ...
        <?php else: ?>
            <div class="article-list">
                ...
                <div class="article-item">
                    ...
                    <div class="article-info"><div class="article-name"><?= $f['pinned'] ? '<span class="pin-badge" title="已置顶"><svg viewBox="0 0 24 24" width="16" height="16"><path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26L12 2z" fill="currentColor" stroke="none"/></svg></span>' : '' ?><?= $f['hidden'] ? '<span class="hidden-badge" title="已隐藏"><svg viewBox="0 0 24 24" width="14" height="14"><rect x="3" y="11" width="18" height="11" rx="2" ry="2" fill="currentColor" stroke="none"/><path d="M7 11V7a5 5 0 0 1 10 0v4" fill="none" stroke="currentColor" stroke-width="2"/></svg></span>' : '' ?><?= htmlspecialchars($f['displayName']) ?></div></div>
                    <div class="article-actions">
                        ...
                        <button class="action-btn hide-btn <?= $f['hidden'] ? 'is-hidden' : '' ?>" title="<?= $f['hidden'] ? '取消隐藏' : '隐藏内容' ?>" data-name="<?= htmlspecialchars($f['name']) ?>" data-hidden="<?= $f['hidden'] ? '1' : '0' ?>"><svg viewBox="0 0 24 24"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg></button>
...
<div class="modal-overlay" id="hideModal">
    <div class="modal-box">
        <h3 id="hideModalTitle">内容隐藏</h3>
        <form method="post" id="hideForm">
            <input type="hidden" name="toggle_hide" id="hideConfirmFile">
            <div class="toggle-row" style="padding-top:0;border-bottom:none;margin-bottom:12px;">
                <div><div class="toggle-label">启用隐藏</div><div class="toggle-desc" id="hideToggleDesc">开启后需密码才能查看</div></div>
                <label class="toggle"><input type="checkbox" id="hideToggle" checked><span class="slider"></span></label>
            </div>
            <div id="hidePwSection">
                <div class="form-group" style="text-align:left;margin-bottom:16px">
                    <label>解锁密码</label>
                    <input type="text" name="hide_password" id="hidePasswordInput" placeholder="输入密码" autocomplete="off">
                </div>
            </div>
            <div class="modal-actions">
                <button type="button" class="btn btn-outline btn-sm" onclick="closeModal('hideModal')">取消</button>
                <button type="submit" class="btn btn-sm" id="hideSubmitBtn">确认</button>
            </div>
        </form>
    </div>
</div>
...
document.querySelectorAll('.hide-btn').forEach(function(btn){
    btn.addEventListener('click',function(){
        ...
        document.getElementById('hideConfirmFile').value = name;
        document.getElementById('hidePasswordInput').value = '';
        var toggle = document.getElementById('hideToggle');
        var pwSection = document.getElementById('hidePwSection');
        var submitBtn = document.getElementById('hideSubmitBtn');
        var toggleDesc = document.getElementById('hideToggleDesc');
        if(isHidden){
            toggle.checked = false;
            pwSection.style.display = 'none';
            toggleDesc.textContent = '当前已隐藏，关闭将取消隐藏';
            submitBtn.textContent = '取消隐藏';
        }else{
            toggle.checked = true;
            pwSection.style.display = 'block';
            toggleDesc.textContent = '开启后需密码才能查看';
            submitBtn.textContent = '确认隐藏';
        }
        toggle.onchange = function(){
            if(toggle.checked){
                ...
                toggleDesc.textContent = '开启后需密码才能查看';
                submitBtn.textContent = '确认隐藏';
                setTimeout(function(){document.getElementById('hidePasswordInput').focus()},100);
            }else{
                pwSection.style.display = 'none';
                toggleDesc.textContent = '当前已隐藏，关闭将取消隐藏';
                submitBtn.textContent = '取消隐藏';
        ...
        if(!isHidden) setTimeout(function(){document.getElementById('hidePasswordInput').focus()},100);
    });
});
</script>
</body>
