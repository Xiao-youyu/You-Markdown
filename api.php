...
function loadSiteConfig() {
    $f = './data/.config.json';
    if (!file_exists($f)) return ['site_title' => 'You Markdown', 'reg_limit_per_ip' => 3, 'comments_enabled' => true, 'auto_ban' => true];
...
function saveBansList($bans) {
    file_put_contents('./data/.bans.json', json_encode($bans, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
}
...
function saveLogsList($logs) {
    file_put_contents('./data/.logs.json', json_encode($logs, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
}
...
function addBan($ip, $types, $reason = '') {
    $bans = loadBansList();
    foreach ($bans as &$b) { if ($b['ip'] === $ip) { foreach ($types as $t) { if (!in_array($t, $b['types'])) $b['types'][] = $t; } return; } }
    unset($b);
...
if ($action === 'register' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $regLimit = max(1, intval($siteCfg['reg_limit_per_ip'] ?? 3));
...
if ($action === 'login' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (count($ipFails) >= 10) {
        ...
        if ($config['auto_ban'] ?? false) addBan($clientIP, ['login'], '自动封禁：频繁错误登录');
