...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $config['music_cookies'] = trim($_POST['music_cookies'] ?? ($config['music_cookies'] ?? ''));
...
<main class="main">
    ...
    <form method="post">
        ...
        <div class="form-card">
            ...
            <div class="form-group">
                <label>网易云 Cookies（可选）</label>
                <input type="text" name="music_cookies" value="<?= htmlspecialchars($config['music_cookies'] ?? '') ?>" placeholder="MUSIC_U=xxx; __csrf=xxx; ...">
                <div class="hint">登录网易云后从浏览器开发者工具 → Network → 任意请求的 Request Headers 中复制 Cookie。配置后可播放 VIP 歌曲</div>
            </div>
        </div>
