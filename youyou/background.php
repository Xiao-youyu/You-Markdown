...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['_bg_save'])) {
    ...
    $config['bg_api_url_mobile'] = trim($_POST['bg_api_url_mobile']??'');
    ...
    $config['widget_glass_enabled'] = !empty($_POST['widget_glass_enabled']);
    $config['card_glass_enabled'] = !empty($_POST['card_glass_enabled']);
    $config['card_glass_level'] = max(4, min(40, intval($_POST['card_glass_level']??16)));
...
$bgApiUrlMobile = $_siteConfig['bg_api_url_mobile'] ?? '';
...
$widgetGlassEnabled = !empty($_siteConfig['widget_glass_enabled']);
$cardGlassEnabled = !empty($_siteConfig['card_glass_enabled']);
$cardGlassLevel = $_siteConfig['card_glass_level'] ?? 16;
$msg = $_GET['msg'] ?? '';
?>
<!DOCTYPE html>
<html lang="zh-CN">
...
<main class="main">
    ...
    <div class="form-card" id="apiSection" style="display:<?= $bgType==='api'?'block':'none' ?>">
        ...
        <div class="form-group">
            ...
            <div class="hint">电脑端访问时使用的背景 API 地址</div>
        </div>
        <div class="form-group">
            <label>手机端 API URL（可选）</label>
            <div class="api-url-group">
                <input type="url" id="bgApiUrlMobile" value="<?= htmlspecialchars($bgApiUrlMobile) ?>" placeholder="留空则和电脑端一致">
                <button class="btn btn-sm" type="button" onclick="testApiUrlMobile()">测试</button>
            </div>
            <div class="hint">手机端访问时使用的背景 API 地址，留空则使用电脑端地址</div>
        </div>
    ...
    <div class="form-card" id="blurSection">
        ...
        <div class="slider-group" style="margin-top:16px">
            ...
            <div class="toggle-row" style="margin-top:14px;">
                <div><div class="toggle-label">小组件毛玻璃</div><div class="toggle-desc">顶部栏、悬浮按钮、展开面板等小组件采用毛玻璃背景</div></div>
                <label class="toggle"><input type="checkbox" id="widgetGlassToggle" <?= $widgetGlassEnabled ? 'checked' : '' ?>><span class="slider"></span></label>
            </div>
            <div class="toggle-row" style="margin-top:14px;">
                <div><div class="toggle-label">卡片毛玻璃</div><div class="toggle-desc">文章卡片、分类栏、阅读区、评论区等采用毛玻璃背景</div></div>
                <label class="toggle"><input type="checkbox" id="cardGlassToggle" <?= $cardGlassEnabled ? 'checked' : '' ?>><span class="slider"></span></label>
            </div>
            <div id="cardGlassLevelWrap" style="display:<?= $cardGlassEnabled ? 'block' : 'none' ?>;padding-top:8px">
                <div class="slider-group">
                    <div class="slider-header"><label>毛玻璃强度</label><span class="slider-val" id="cardGlassVal"><?= $cardGlassLevel ?>px</span></div>
                    <div class="slider-row"><span class="slider-label">弱</span><input type="range" min="4" max="40" value="<?= $cardGlassLevel ?>" step="2" id="cardGlassSlider"><span class="slider-label">强</span></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <form method="post" id="bgForm">
        ...
        <input type="hidden" name="bg_api_url_mobile" id="formBgApiUrlMobile" value="<?= htmlspecialchars($bgApiUrlMobile) ?>">
        ...
        <input type="hidden" name="widget_glass_enabled" id="formWidgetGlass" value="<?= $widgetGlassEnabled ? '1' : '' ?>">
        <input type="hidden" name="card_glass_level" id="formCardGlassLevel" value="<?= $cardGlassLevel ?>">
        <input type="hidden" name="card_glass_enabled" id="formCardGlass" value="<?= $cardGlassEnabled ? '1' : '' ?>">
...
(function() {
    ...
    window.testApiUrlMobile = function() {
        var url = document.getElementById('bgApiUrlMobile').value.trim();
        if (!url) return;
        var result = document.getElementById('apiTestResult');
        result.innerHTML = '<span style="color:var(--text-muted);font-size:13px">测试中...</span>';
        var img = new Image();
        img.onload = function() {
            result.innerHTML = '<div class="img-preview-thumb"><img src="'+url+'" style="max-width:200px;max-height:120px"></div><div style="font-size:12px;color:#16a34a;margin-top:4px">✓ 手机端 API 可用</div>';
        };
    ...
    var cardGlassSlider = document.getElementById('cardGlassSlider');
    var cardGlassVal = document.getElementById('cardGlassVal');
    var cardGlassLevelWrap = document.getElementById('cardGlassLevelWrap');
    cardGlassToggle.addEventListener('change', function() { cardGlassLevelWrap.style.display = cardGlassToggle.checked ? 'block' : 'none'; updatePreview(); });
    cardGlassSlider.addEventListener('input', function() { cardGlassVal.textContent = cardGlassSlider.value + 'px'; updatePreview(); });
    function updatePreview() {
        ...
        var cardGlassOn = cardGlassToggle.checked;
        var cardGlassBlur = cardGlassOn ? parseInt(cardGlassSlider.value) : 0;
        if (cardGlassOn) {
            previewCard.style.background = 'rgba(255,255,255,0.55)';
            previewCard.style.backdropFilter = 'blur(' + cardGlassBlur + 'px) saturate(1.8)';
            previewCard.style.webkitBackdropFilter = 'blur(' + cardGlassBlur + 'px) saturate(1.8)';
            previewCard.style.border = '1px solid rgba(255,255,255,0.4)';
        } else {
        ...
        if (cardGlassOn) labels.push('毛玻璃 ' + cardGlassBlur + 'px');
    ...
    window.resetBg = function() {
        ...
        cardGlassToggle.checked = false; cardGlassSlider.value = 16; cardGlassVal.textContent = '16px';
        cardGlassLevelWrap.style.display = 'none';
        updatePreview();
    };
    document.getElementById('bgForm').addEventListener('submit', function() {
        ...
        document.getElementById('formBgApiUrlMobile').value = (document.getElementById('bgApiUrlMobile')||{}).value||''.trim();
        document.getElementById('formBlurEnabled').value = blurToggle.checked ? '1' : '';
        document.getElementById('formWidgetGlass').value = document.getElementById('widgetGlassToggle').checked ? '1' : '';
        document.getElementById('formCardGlass').value = document.getElementById('cardGlassToggle').checked ? '1' : '';
        document.getElementById('formCardGlassLevel').value = cardGlassSlider.value;
