...
function ymCopyDir($src, $dst, $skipDirs, $skipFiles) {
    ...
    while (($file = readdir($dir)) !== false) {
        ...
        if (is_dir($srcPath)) {
            ...
            ymCopyDir($srcPath, $dstPath, $skipDirs, $skipFiles);
        } else {
            if (in_array($file, $skipFiles)) continue;
            if (!@copy($srcPath, $dstPath)) {
                error_log('You-Markdown update: copy failed: ' . $srcPath . ' -> ' . $dstPath);
            }
        }
    }
    closedir($dir);
...
function ymRemoveDir($dir) {
    if (!is_dir($dir)) return;
    foreach (scandir($dir) as $f) {
        if ($f === '.' || $f === '..') continue;
        $p = $dir . '/' . $f;
        is_dir($p) ? ymRemoveDir($p) : unlink($p);
    }
    rmdir($dir);
}
if (isset($_GET['ajax'])) {
    ...
    if ($action === 'update') {
        ...
        $ok = ymCopyDir($srcDir, $projectRoot, $skipDirs, $skipFiles);
        ymRemoveDir($tmpDir);
        if ($ok) {
    ...
    if ($action === 'manual_update') {
        if (!isset($_FILES['update_zip'])) {
            ...
            if (!empty($_SERVER['CONTENT_LENGTH'])) {
                ...
                $reason .= '（可能是文件超限：post_max_size=' . $maxPost . ', upload_max_filesize=' . $maxUpload . ', Content-Length=' . $_SERVER['CONTENT_LENGTH'] . '）';
            }
            echo json_encode(['success' => false, 'error' => '文件未上传：' . $reason], JSON_UNESCAPED_UNICODE);
            exit;
        }
        if ($_FILES['update_zip']['error'] !== UPLOAD_ERR_OK) {
            $errs = [1=>'文件超过 upload_max_filesize', 2=>'文件超过表单限制', 3=>'文件部分上传',4=>'没有文件被上传',6=>'缺少临时目录',7=>'写入磁盘失败',8=>'PHP 扩展阻止了上传'];
            $code = $_FILES['update_zip']['error'];
            echo json_encode(['success' => false, 'error' => '上传错误(' . $code . ')：' . ($errs[$code] ?? '未知错误')], JSON_UNESCAPED_UNICODE);
            exit;
        }
        $file = $_FILES['update_zip'];
        $ext = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION));
        if ($ext !== 'zip') {
            echo json_encode(['success' => false, 'error' => '仅支持 .zip 格式'], JSON_UNESCAPED_UNICODE);
            exit;
        }
        if ($file['size'] > 50 * 1024 * 1024) {
            echo json_encode(['success' => false, 'error' => '文件过大，最大 50MB'], JSON_UNESCAPED_UNICODE);
            exit;
        }
        $tmpDir = sys_get_temp_dir() . '/ym_manual_' . uniqid();
        $zipFile = $tmpDir . '.zip';
        if (!move_uploaded_file($file['tmp_name'], $zipFile)) {
            echo json_encode(['success' => false, 'error' => '临时文件保存失败'], JSON_UNESCAPED_UNICODE);
            exit;
        }
        $zip = new ZipArchive();
        if ($zip->open($zipFile) !== true) {
            @unlink($zipFile);
            echo json_encode(['success' => false, 'error' => 'ZIP 文件解压失败'], JSON_UNESCAPED_UNICODE);
            exit;
        }
        mkdir($tmpDir, 0755, true);
        $zip->extractTo($tmpDir);
        ...
        if ($ok) {
            ...
            $debug = 'srcDir=' . $srcDir . (is_dir($srcDir) ? '(存在)' : '(不存在)') . ' projectRoot=' . $projectRoot . (is_dir($projectRoot) ? '(存在)' : '(不存在)');
            echo json_encode(['success' => false, 'error' => '文件复制失败，' . $debug], JSON_UNESCAPED_UNICODE);
        }
        exit;
    }
...
:root{--accent-hue:220;--accent-sat:60%;--accent:hsl(var(--accent-hue),var(--accent-sat),50%);--bg:hsl(var(--accent-hue),60%,96%);--surface:
...
.toggle .slider{position:absolute;inset:0;background:
.toggle .slider:before{content:'';position:absolute;width:18px;height:18px;border-radius:50%;background:
...
.spinner{display:inline-block;width:16px;height:16px;border:2px solid rgba(255,255,255,.3);border-top-color:
...
.badge.stable{background:
.badge.beta{background:
.badge.has-update{background:
...
[data-theme="dark"]{--bg:hsl(var(--accent-hue),40%,8%);--surface:
</style>
</head>
<body>
...
<div class="modal-mask" id="manualUpdateModal">
    <div class="modal-box">
        <div class="modal-head">
            <div class="modal-title">手动更新</div>
        </div>
        <div class="modal-body">
            <p>选择本地 ZIP 更新包进行手动更新。<br><strong>data</strong> 目录和自定义字体不会被覆盖。</p>
            <div id="manualUploadArea" style="border:2px dashed var(--border);border-radius:12px;padding:28px 16px;text-align:center;cursor:pointer;transition:all .2s;margin-top:12px;">
                <svg viewBox="0 0 24 24" width="36" height="36" style="stroke:var(--text-muted);fill:none;stroke-width:2;stroke-linecap:round;stroke-linejoin:round;margin-bottom:8px;display:block;margin-left:auto;margin-right:auto"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
                <div id="manualUploadText" style="font-size:.9em;color:var(--text-muted)">点击选择 ZIP 文件，或拖拽到此处</div>
                <div id="manualFileName" style="font-size:.85em;color:var(--accent);margin-top:6px;display:none;font-weight:600"></div>
                <input type="file" id="manualFileInput" accept=".zip" style="display:none">
            </div>
            <div id="manualProgress" style="display:none;margin-top:14px">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;font-size:.85em">
                    <span id="manualProgressText" style="color:var(--text-secondary)">上传中...</span>
                    <span id="manualProgressPct" style="color:var(--accent);font-weight:600">0%</span>
                </div>
                <div style="height:6px;background:var(--bg);border-radius:3px;overflow:hidden">
                    <div id="manualProgressBar" style="height:100%;background:var(--accent);border-radius:3px;width:0%;transition:width .2s"></div>
                </div>
            </div>
        </div>
        <div class="modal-actions">
            <button class="btn btn-outline btn-sm" onclick="closeManualUpdate()">取消</button>
            <button class="btn btn-sm" id="manualUpdateBtn" disabled onclick="doManualUpdate()">开始更新</button>
        </div>
    </div>
</div>
...
var latestVersion = '';
var stableClickCount = 0;
var stableClickTimer = null;
function showManualUpdate() {
    document.getElementById('manualUpdateModal').classList.add('show');
    resetManualUpload();
}
function closeManualUpdate() {
    document.getElementById('manualUpdateModal').classList.remove('show');
    resetManualUpload();
}
function resetManualUpload() {
    document.getElementById('manualFileInput').value = '';
    document.getElementById('manualFileName').style.display = 'none';
    document.getElementById('manualFileName').textContent = '';
    document.getElementById('manualUploadText').textContent = '点击选择 ZIP 文件，或拖拽到此处';
    document.getElementById('manualUploadArea').style.borderColor = 'var(--border)';
    document.getElementById('manualProgress').style.display = 'none';
    document.getElementById('manualProgressBar').style.width = '0%';
    document.getElementById('manualUpdateBtn').disabled = true;
    selectedManualFile = null;
}
var selectedManualFile = null;
var uploadArea = document.getElementById('manualUploadArea');
var fileInput = document.getElementById('manualFileInput');
uploadArea.addEventListener('click', function() { fileInput.click(); });
uploadArea.addEventListener('dragover', function(e) { e.preventDefault(); this.style.borderColor = 'var(--accent)'; });
uploadArea.addEventListener('dragleave', function() { this.style.borderColor = 'var(--border)'; });
...
fileInput.addEventListener('change', function() {
    if (this.files[0]) handleManualFile(this.files[0]);
});
function handleManualFile(f) {
    selectedManualFile = f;
    document.getElementById('manualFileName').textContent = f.name + ' (' + (f.size / 1024 / 1024).toFixed(1) + 'MB)';
    document.getElementById('manualFileName').style.display = 'block';
    document.getElementById('manualUploadText').textContent = '点击更换文件';
    document.getElementById('manualUpdateBtn').disabled = false;
}
function doManualUpdate() {
    if (!selectedManualFile) return;
    if (!confirm('确定要使用本地 ZIP 包更新吗？\n\ndata 目录和自定义字体不会被覆盖。')) return;
    var btn = document.getElementById('manualUpdateBtn');
    btn.disabled = true;
    btn.textContent = '上传中...';
    document.getElementById('manualProgress').style.display = 'block';
    var formData = new FormData();
    formData.append('update_zip', selectedManualFile);
    formData.append('ajax', 'manual_update');
    var xhr = new XMLHttpRequest();
    xhr.open('POST', 'update.php?ajax=manual_update', true);
    xhr.upload.addEventListener('progress', function(e) {
        if (e.lengthComputable) {
            var pct = Math.round(e.loaded / e.total * 100);
            document.getElementById('manualProgressBar').style.width = pct + '%';
            document.getElementById('manualProgressPct').textContent = pct + '%';
            if (pct >= 100) {
                document.getElementById('manualProgressText').textContent = '正在安装...';
            }
        }
    });
    xhr.addEventListener('load', function() {
        ...
        try {
            ...
            showToast('更新失败（' + xhr.status + '）：' + xhr.responseText.substring(0, 200));
            btn.disabled = false;
        }
    });
...
function switchChannel(ch) {
    if (ch === 'stable') {
        stableClickCount++;
        clearTimeout(stableClickTimer);
        stableClickTimer = setTimeout(function() { stableClickCount = 0; }, 1500);
        if (stableClickCount >= 5) {
            stableClickCount = 0;
            clearTimeout(stableClickTimer);
            showManualUpdate();
            return;
