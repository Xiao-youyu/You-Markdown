(function() {
    (function applyBg() {
        ...
        var bgApiUrlMobile = body.dataset.bgApiUrlMobile || '';
        ...
        var activeApiUrl = (isMobile && bgApiUrlMobile) ? bgApiUrlMobile : bgApiUrl;
        ...
        var widgetGlass = body.dataset.widgetGlass === '1';
        if (widgetGlass) {
            body.classList.add('widget-glass');
        }
        var cardGlass = body.dataset.cardGlass === '1';
        if (cardGlass) {
            body.classList.add('card-glass');
            var cardGlassLevel = parseInt(body.dataset.cardGlassLevel || '16');
            body.style.setProperty('--card-glass-blur', cardGlassLevel + 'px');
        }
        if (bgType === 'image' && bgImage) {
            body.classList.add('bg-active');
            body.style.setProperty('--bg-url', 'url(' + bgImage + ')');
        } else if (bgType === 'api' && activeApiUrl) {
            body.classList.add('bg-active');
            body.style.setProperty('--bg-url', 'url(' + activeApiUrl + ')');
        }
    })();
    ...
    if (colorResetBtn) colorResetBtn.addEventListener('click', () => { setAccentHue(220); updateHueThumb(220); });
    ...
    function updateHueThumb(hue) {
        ...
        colorPreviewSwatch.style.background = 'hsl(' + hue + ', 60%, 50%)';
        colorPreviewLabel.textContent = 'hsl(' + hue + ', 60%, 50%)';
    }
    function handleHueTrackInteraction(e) {
    ...
    hueTrack.addEventListener('mousedown', function(e) {
        handleHueTrackInteraction(e);
        function onMove(ev) { handleHueTrackInteraction(ev); }
        function onUp() { document.removeEventListener('mousemove', onMove); document.removeEventListener('mouseup', onUp); }
        document.addEventListener('mousemove', onMove);
        document.addEventListener('mouseup', onUp);
    });
    hueTrack.addEventListener('touchstart', function(e) {
        handleHueTrackInteraction(e.touches[0]);
        function onMove(ev) { ev.preventDefault(); handleHueTrackInteraction(ev.touches[0]); }
    ...
    updateHueThumb(parseInt(savedHue));
    ...
    shareModalClose.addEventListener('click', () => { shareModalOverlay.classList.add('closing');
        setTimeout(function() { shareModalOverlay.classList.remove('active', 'closing'); }, 200); shareQrcode.innerHTML = ''; });
    shareModalOverlay.addEventListener('click', (e) => { if (e.target === shareModalOverlay) { shareModalOverlay.classList.add('closing');
        setTimeout(function() { shareModalOverlay.classList.remove('active', 'closing'); }, 200); shareQrcode.innerHTML = ''; } });
    ...
    function renderCards() {
        ...
        var bgType = document.body.dataset.bgType || 'none';
        var bgOpacity = parseInt(document.body.dataset.bgCardOpacity || '100');
        var showCover = bgType === 'none' && bgOpacity === 100;
        ...
        cardsGrid.innerHTML = displayFiles.map((file, i) => {
            var coverHTML = '';
            if (showCover && file.coverImage) {
                coverHTML = `<div class="card-cover"><img src="${escapeHTML(file.coverImage)}" alt="" loading="lazy" onerror="this.parentElement.style.display='none'"></div>`;
            }
            return `
            <div class="doc-card${showCover && file.coverImage ? ' has-cover' : ''}" data-filename="${escapeHTML(file.name)}" style="animation-delay:${i*0.05}s">
                <div class="card-body">
                    <div class="card-title">${file.pinned ? '<span class="card-pin-icon" title="置顶"><svg viewBox="0 0 24 24" width="18" height="18"><path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26L12 2z" fill="currentColor" stroke="none"/></svg></span>' : ''}${file.hidden ? '<span class="card-lock-icon" title="已隐藏"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg></span>' : ''}${escapeHTML(file.displayName)}</div>
                    <div class="card-meta">
        ...
        sidebarFileList.innerHTML = files.map(file => `
            <div class="sidebar-item" data-filename="${escapeHTML(file.name)}">
                ...
                <div style="flex:1;min-width:0;overflow:hidden;">
                    <div class="sidebar-item-text">${file.pinned ? '<span class="sidebar-pin-icon" title="置顶"><svg viewBox="0 0 24 24" width="14" height="14"><path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26L12 2z" fill="currentColor" stroke="none"/></svg></span>' : ''}${file.hidden ? '<span class="sidebar-lock-icon" title="已隐藏"><svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg></span>' : ''}${escapeHTML(file.displayName)}</div>
    ...
    function showHome(pushState = true) {
        ...
        tocPopup.classList.remove('active'); musicPopup.classList.remove('active'); shareModalOverlay.classList.add('closing');
        setTimeout(function() { shareModalOverlay.classList.remove('active', 'closing'); }, 200); shareQrcode.innerHTML = '';
    ...
    function showHiddenPasswordUI(filename, fileMeta) {
        showReading();
        markdownBody.innerHTML =
            '<div class="hidden-article-container">' +
                '<div class="hidden-article-card">' +
                    '<div class="hidden-lock-ring">' +
                        '<svg class="hidden-lock-icon" viewBox="0 0 24 24" width="32" height="32" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">' +
                            '<rect x="3" y="11" width="18" height="11" rx="2" ry="2"/>' +
                            '<path d="M7 11V7a5 5 0 0 1 10 0v4"/>' +
                        '</svg>' +
                    '</div>' +
                    '<div class="hidden-article-title">' + escapeHTML(fileMeta.displayName) + '</div>' +
                    '<div class="hidden-article-subtitle">此内容已加密，请输入密码解锁</div>' +
                    '<div class="hidden-pw-form">' +
                        '<div class="hidden-pw-input-wrap">' +
                            '<svg class="hidden-pw-key-icon" viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">' +
                                '<path d="M21 2l-2 2m-7.61 7.61a5.5 5.5 0 1 1-7.778 7.778 5.5 5.5 0 0 1 7.777-7.777zm0 0L15.5 7.5m0 0l3 3L22 7l-3-3m-3.5 3.5L19 4"/>' +
                            '</svg>' +
                            '<input type="password" id="hiddenPwInput" placeholder="输入密码" autocomplete="off" class="hidden-pw-input">' +
                        '</div>' +
                        '<div id="hiddenPwError" class="hidden-pw-error"></div>' +
                        '<button id="hiddenPwBtn" class="hidden-pw-btn">' +
                            '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="13 17 18 12 13 7"/><line x1="6" y1="12" x2="18" y2="12"/></svg>' +
                            '<span>解锁</span>' +
                        '</button>' +
                    '</div>' +
                '</div>' +
            '</div>';
        document.title = fileMeta.displayName + ' - 已隐藏 | You Markdown';
        cmtOnArticleLoad();
        const pwInput = document.getElementById('hiddenPwInput');
        const pwBtn = document.getElementById('hiddenPwBtn');
        const pwError = document.getElementById('hiddenPwError');
        pwInput.focus();
        function showError(msg) {
            pwError.textContent = msg;
            pwError.classList.add('show');
            pwError.style.animation = 'none';
            pwError.offsetHeight;
        ...
        async function submitPassword() {
            ...
            hideError();
            pwBtn.disabled = true;
            pwBtn.innerHTML = '<span class="hidden-pw-spinner"></span><span>验证中...</span>';
            try {
                const resp = await fetch('?action=verify_article_password', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({file: filename, password: pw})
                });
            ...
            pwBtn.innerHTML = '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="13 17 18 12 13 7"/><line x1="6" y1="12" x2="18" y2="12"/></svg><span>解锁</span>';
        }
    ...
    async function loadFile(filename, pushState = true) {
        ...
        try {
            ...
            if (data.success) {
                ...
                // Handle hidden article
                if (data.hidden) {
                    showHiddenPasswordUI(filename, fileMeta);
                    return;
                }
    ...
    async function init() {
        ...
        if (getUrlParam('oauth_success') === '1') {
            cmtCheckAuth().then(() => {
                if (cmtUser) {
                    showToast('OAuth 登录成功！');
                    cmtCloseModal(cmtAuthModal);
                    if (currentFileName) cmtLoad();
                } else {
                    showToast('登录会话异常，请重试');
                }
                history.replaceState(null, '', location.pathname + (fileParam ? '?file=' + encodeURIComponent(fileParam) : ''));
            });
        } else if (getUrlParam('oauth_error')) {
            const errCode = getUrlParam('oauth_error');
            const errMap = {
                'invalid_state': '授权状态无效，请重试',
                'no_code': '未收到授权码',
                'token_failed': '获取 Token 失败，请检查网络',
                'token_invalid': 'Token 无效',
                'verify_failed': '验证失败',
                'verify_invalid': '验证无效',
                'no_user': '无法获取用户信息',
                'access_denied': '用户取消了授权',
                'invalid_client': '应用配置错误，请联系站长',
                'invalid_grant': '授权码无效或已过期，请重试',
                'server_error': 'OAuth 服务异常，请稍后重试',
                'config_missing': 'OAuth 未配置，请联系站长'
            };
            const errMsg = errMap[errCode] || ('OAuth 登录失败（' + decodeURIComponent(errCode) + '）');
    ...
    function closeLightbox() {
        ...
        setTimeout(function() {
            imgLightbox.classList.remove('active', 'closing');
    ...
    const emailVerifyEnabled = document.body.dataset.emailVerify === '1';
    const cmtEmailVerifyModal = document.getElementById('cmtEmailVerifyModal');
    const cmtVerifyEmail = document.getElementById('cmtVerifyEmail');
    const cmtVerifyCode = document.getElementById('cmtVerifyCode');
    const cmtVerifySendCode = document.getElementById('cmtVerifySendCode');
    const cmtVerifyErr = document.getElementById('cmtVerifyErr');
    const cmtVerifySubmit = document.getElementById('cmtVerifySubmit');
    let cmtPendingReg = null;
    const cmtSwitchText = document.getElementById('cmtSwitchText');
    const cmtSwitchBtn = document.getElementById('cmtSwitchBtn');
    const cmtToRegBtn = document.getElementById('cmtToRegBtn');
    const cmtToLoginBtn = document.getElementById('cmtToLoginBtn');
    ...
    function showCustomConfirm(msg, cb) {
        var overlay = document.getElementById('cmtConfirmOverlay');
        var textEl = document.getElementById('cmtConfirmText');
        var okBtn = document.getElementById('cmtConfirmOk');
    ...
    function cmtAvatarHtml(url, name, qq, uid) {
        const initial = cmtEscape((name||'?').charAt(0));
        const src = uid ? 'api.php?action=avatar&uid=' + encodeURIComponent(uid) : (qq ? 'api.php?action=avatar&qq=' + encodeURIComponent(qq) : (url || ''));
    ...
    function cmtCloseModal(id) {
        if (!id || !id.classList.contains('show')) return;
        id.classList.add('closing');
        setTimeout(function() { id.classList.remove('show', 'closing'); }, 200);
    }
    [cmtAuthModal, cmtProfileModal, cmtAdminModal, cmtEmailVerifyModal].forEach(m => {
    ...
    if (cmtCapsuleBtn) cmtCapsuleBtn.addEventListener('click', () => {
        cmtAuthMode = 'login'; cmtUpdateAuthUI(); cmtOpenModal(cmtAuthModal);
        var yyBtn = document.getElementById('cmtYouyunBtn');
        if (yyBtn) { yyBtn.disabled = false; var oauthName = yyBtn.dataset.oauthName || 'OAuth'; var oauthIcon = yyBtn.dataset.oauthIcon || ''; if (oauthIcon) { yyBtn.innerHTML = '<img src="' + oauthIcon + '" width="18" height="18" style="border-radius:4px;object-fit:cover;"> ' + oauthName + ' 账号登录'; } else { yyBtn.innerHTML = '<svg width="18" height="18" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="1.5"/><path d="M12 7v5l3 2" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg> ' + oauthName + ' 账号登录'; } }
    });
    ...
    if (cmtLogoutBtn) cmtLogoutBtn.addEventListener('click', e => {
        e.stopPropagation();
        showCustomConfirm('确定要退出登录吗？', function() {
            fetch('api.php?action=logout', { method: 'POST' }).catch(() => {});
            cmtUser = null;
            cmtUpdateUI();
        });
    });
    function cmtSwitchTo(mode) {
    ...
    if (cmtToRegBtn) cmtToRegBtn.addEventListener('click', () => cmtSwitchTo('register'));
    if (cmtToLoginBtn) cmtToLoginBtn.addEventListener('click', () => cmtSwitchTo('login'));
    if (cmtSwitchBtn) cmtSwitchBtn.addEventListener('click', () => cmtSwitchTo(cmtAuthMode === 'login' ? 'register' : 'login'));
    const cmtYouyunBtn = document.getElementById('cmtYouyunBtn');
    if (cmtYouyunBtn) cmtYouyunBtn.addEventListener('click', () => {
        cmtYouyunBtn.disabled = true;
        var origHTML = cmtYouyunBtn.innerHTML;
        cmtYouyunBtn.textContent = '正在跳转...';
        fetch('api.php?action=oauth_login', { method: 'POST' })
            .then(function(r) { return r.json(); }).then(function(d) {
                if (d.success && d.auth_url) {
                    window.location.href = d.auth_url;
                } else {
                    showToast(d.error || '获取授权链接失败');
                    cmtYouyunBtn.disabled = false;
                    cmtYouyunBtn.innerHTML = origHTML;
                }
            }).catch(function() {
                showToast('网络错误');
                cmtYouyunBtn.disabled = false;
    ...
    if (cmtRegBtn) cmtRegBtn.addEventListener('click', () => {
        ...
        if (!emailVerifyEnabled) {
            ...
            fetch('api.php?action=register', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ qq, nickname: nick, password: pw }) })
                ...
                .finally(() => { cmtRegBtn.disabled = false; cmtRegBtn.textContent = '立即注册'; });
        } else {
            cmtPendingReg = { qq, nickname: nick, password: pw };
            if (cmtVerifyEmail) cmtVerifyEmail.value = '';
            if (cmtVerifyCode) cmtVerifyCode.value = '';
            if (cmtVerifyErr) cmtVerifyErr.textContent = '';
            if (cmtVerifySendCode) { cmtVerifySendCode.disabled = false; cmtVerifySendCode.textContent = '发送验证码'; }
            cmtCloseModal(cmtAuthModal);
            cmtOpenModal(cmtEmailVerifyModal);
        }
    });


    // Email verify modal: send code
    if (cmtVerifySendCode) cmtVerifySendCode.addEventListener('click', () => {
        const email = cmtVerifyEmail ? cmtVerifyEmail.value.trim() : '';
        if (!email) { cmtVerifyErr.textContent = '请填写邮箱'; return; }
        cmtVerifyErr.textContent = '';
        cmtVerifySendCode.disabled = true;
        let countdown = 60;
        cmtVerifySendCode.textContent = countdown + 's';
        const timer = setInterval(() => { countdown--; cmtVerifySendCode.textContent = countdown + 's'; if (countdown <= 0) { clearInterval(timer); cmtVerifySendCode.disabled = false; cmtVerifySendCode.textContent = '发送验证码'; } }, 1000);
        fetch('api.php?action=send_email_code', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email }) })
            .then(r => r.json()).then(d => {
                if (!d.success) { cmtVerifyErr.textContent = d.error || '发送失败'; clearInterval(timer); cmtVerifySendCode.disabled = false; cmtVerifySendCode.textContent = '发送验证码'; }
            }).catch(() => { cmtVerifyErr.textContent = '网络错误'; clearInterval(timer); cmtVerifySendCode.disabled = false; cmtVerifySendCode.textContent = '发送验证码'; });
    });

    // Email verify modal: verify and register
    if (cmtVerifySubmit) cmtVerifySubmit.addEventListener('click', () => {
        if (!cmtPendingReg) { cmtVerifyErr.textContent = '注册信息丢失，请重新填写'; return; }
        const email = cmtVerifyEmail ? cmtVerifyEmail.value.trim() : '';
        const emailCode = cmtVerifyCode ? cmtVerifyCode.value.trim() : '';
        cmtVerifyErr.textContent = '';
        if (!email) { cmtVerifyErr.textContent = '请填写邮箱'; return; }
        if (!emailCode) { cmtVerifyErr.textContent = '请输入验证码'; return; }
        cmtVerifySubmit.disabled = true; cmtVerifySubmit.textContent = '验证中...';
        const regData = { qq: cmtPendingReg.qq, nickname: cmtPendingReg.nickname, password: cmtPendingReg.password, email: email, email_code: emailCode };
        fetch('api.php?action=register', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(regData) })
            .then(r => r.json()).then(d => {
                if (d.success) {
                    cmtPendingReg = null;
                    cmtUser = d.user;
                    cmtCloseModal(cmtEmailVerifyModal);
                    cmtUpdateUI(); cmtLoad();
                } else {
                    cmtVerifyErr.textContent = d.error || '注册失败';
                }
            }).catch(() => { cmtVerifyErr.textContent = '网络错误'; })
            .finally(() => { cmtVerifySubmit.disabled = false; cmtVerifySubmit.textContent = '验证并注册'; });
    });

    // Email verify modal: close on backdrop click (cancel registration)
    if (cmtEmailVerifyModal) cmtEmailVerifyModal.addEventListener('click', (e) => {
        if (e.target === cmtEmailVerifyModal) {
            cmtPendingReg = null;
            cmtCloseModal(cmtEmailVerifyModal);
        }
    });
    ...
    function cmtUpdateUI() {
        ...
        if (loggedIn) {
            ...
            if (cmtUserAvatar) {
                const avatarSrc = cmtUser.id ? 'api.php?action=avatar&uid=' + encodeURIComponent(cmtUser.id) : (cmtUser.qq ? 'api.php?action=avatar&qq=' + encodeURIComponent(cmtUser.qq) : (cmtUser.avatar || ''));
                if (avatarSrc) {
    ...
    function cmtRenderReply(r, level, parentNick) {
        ...
        return '<div class="cmt-reply-item" data-id="' + r.id + '" data-del="' + rDel + '">' +
            '<div class="cmt-reply-top"><div class="cmt-reply-avatar">' + cmtAvatarHtml(r.avatar, r.nickname, r.qq, r.user_id) + '</div>' +
    ...
    function cmtRender(comments) {
        ...
        cmtList.innerHTML = comments.map(c => {
            ...
            return '<div class="cmt-item" data-id="' + c.id + '" data-del="' + canDel + '">' +
                '<div class="cmt-top' + noSignCls + '"><div class="cmt-avatar">' + cmtAvatarHtml(c.avatar, c.nickname, c.qq, c.user_id) + '</div>' +
    ...
    if (cmtConfirmOk) cmtConfirmOk.addEventListener('click', () => {
        if (cmtConfirmCb) cmtConfirmCb();
        cmtConfirmOverlay.classList.add('closing');
        setTimeout(function() { cmtConfirmOverlay.classList.remove('show', 'closing'); cmtConfirmCb = null; }, 200);
    });
    if (cmtConfirmCancel) cmtConfirmCancel.addEventListener('click', () => {
        cmtConfirmOverlay.classList.add('closing');
        setTimeout(function() { cmtConfirmOverlay.classList.remove('show', 'closing'); cmtConfirmCb = null; }, 200);
    });
            ...
            .then(function(data) {
                if (Array.isArray(data) && data.length > 0) {
                    ...
                    if (saved && saved.index >= 0 && saved.index < musicSongs.length) {
                        ...
                        musicList.classList.toggle('open', musicListOpen);
                        musicListToggle.classList.toggle('open', musicListOpen);
                        musicRestoreSong(saved.index, saved.time || 0);
                    } else {
                        musicListOpen = true;
    ...
    function musicGetPlayUrl(s) {
        if (s.url && s.url.indexOf('/outer/url?id=') === -1) {
            // CDN URL 转 HTTPS，浏览器直接播放，不走代理
            return s.url.replace(/^http:\/\//, 'https://');
        }
        return 'music.php?proxy=' + s.id;
    }
    function musicRestoreSong(idx, seekTime) {
        ...
        musicAudio.src = musicGetPlayUrl(s);
        if (seekTime > 0) {
            musicAudio.addEventListener('loadedmetadata', function onMeta() {
    ...
    function musicTryFallbackUrl(song, seekTime) {
        var fallbackUrl = 'music.php?proxy=' + song.id;
        // 直接用 audio 元素流式播放，不下载整个文件
        musicAudio.src = fallbackUrl;
        musicAudio.play().then(function() {
            musicSetPlaying(true);
            musicConsecutiveFails = 0;
            if (seekTime > 0) musicAudio.currentTime = seekTime;
            ...
            musicSetPlaying(false);
            if (musicConsecutiveFails >= 3) {
                musicConsecutiveFails = 0;
                return;
            }
            if (musicSongs.length > 1) {
                var nextIdx = (musicIndex + 1) % musicSongs.length;
                if (nextIdx === 0 && musicConsecutiveFails > 0) {
                    musicConsecutiveFails = 0;
                    return;
                }
                setTimeout(function() { musicPlaySong(nextIdx); }, 800);
            }
        });
    }
    ...
    musicAudio.addEventListener('seeked', function() { _karaoActiveIdx = -1; _karaoFillIdx = -1; _karaoLastLineIdx = -1; });
    musicAudio.addEventListener('ended', function() {
        if (musicAudio.duration > 0 && musicAudio.currentTime < 1) {
            musicConsecutiveFails++;
            if (musicConsecutiveFails >= 3) {
                musicConsecutiveFails = 0;
                musicSetPlaying(false);
    ...
    function musicRenderLyric() {
        _karaoCachedLines = null;
        _karaoCachedSpans = null;
        _karaoLastLineIdx = -1;
        if (!musicLyrLines.length) {
            musicLyrScroll.innerHTML = '<div class="music-lyric-hint">暂无歌词</div>';
            return;
        }
        var html = '<div class="music-lyric-inner">';
    ...
    function musicSetAllWordsGray(lineEl) {
        var spans = lineEl.querySelectorAll('.lyr-word.sung, .lyr-word.filling');
    ...
    var _karaoActiveIdx = -1;
    var _karaoFillIdx = -1;
    var _karaoCachedLines = null;
    var _karaoCachedSpans = null;
    var _karaoLastLineIdx = -1;
    ...
    function musicWordAnimLoop() {
        ...
        // 快速定位当前行：缓存 + 二分
        var activeIdx = -1;
        if (_karaoActiveIdx >= 0 && _karaoActiveIdx < musicLyrLines.length && ct >= musicLyrLines[_karaoActiveIdx].time) {
            activeIdx = _karaoActiveIdx;
            while (activeIdx + 1 < musicLyrLines.length && ct >= musicLyrLines[activeIdx + 1].time) activeIdx++;
        } else {
            var lo = 0, hi = musicLyrLines.length - 1;
            while (lo <= hi) {
                var mid = (lo + hi) >> 1;
                if (musicLyrLines[mid].time <= ct) { activeIdx = mid; lo = mid + 1; } else { hi = mid - 1; }
            }
        }

        if (!_karaoCachedLines) _karaoCachedLines = musicLyrScroll.querySelectorAll('.music-lyric-line');
        var lines = _karaoCachedLines;

        // 切换行：重置旧行
        if (activeIdx !== _karaoActiveIdx) {
            if (_karaoActiveIdx >= 0 && _karaoActiveIdx < lines.length) {
                var oldSpans = lines[_karaoActiveIdx].querySelectorAll('.lyr-word.sung, .lyr-word.filling');
                for (var j = 0; j < oldSpans.length; j++) {
                    oldSpans[j].className = 'lyr-word';
                    oldSpans[j].style.removeProperty('--fill-pct');
                }
            }
            _karaoActiveIdx = activeIdx;
            _karaoFillIdx = -1;
            _karaoLastLineIdx = -1;
        }
        ...
        if (activeIdx >= 0 && activeIdx < lines.length && musicLyrLines[activeIdx].words) {
            ...
            if (_karaoLastLineIdx !== activeIdx) {
                _karaoCachedSpans = lines[activeIdx].querySelectorAll('.lyr-word');
                _karaoLastLineIdx = activeIdx;
            }
            var spans = _karaoCachedSpans;
            var newFillIdx = -1;
            ...
            for (var i = 0; i < words.length; i++) {
                ...
                if (ct >= wordEnd) {
                    // 已唱完：只在状态变化时改
                    if (!spans[i].classList.contains('sung')) {
                    ...
                    if (!spans[i].classList.contains('filling')) spans[i].className = 'lyr-word filling';
                    spans[i].style.setProperty('--fill-pct', pct + '%');
                } else {
                    // 未唱：只在状态不对时重置
                    if (spans[i].className !== 'lyr-word') {
                        spans[i].className = 'lyr-word';
                        spans[i].style.removeProperty('--fill-pct');
                    }
                }
            ...
            // 旧filling字切到sung
            if (_karaoFillIdx >= 0 && _karaoFillIdx !== newFillIdx && _karaoFillIdx < spans.length) {
                if (spans[_karaoFillIdx].classList.contains('filling')) {
                    spans[_karaoFillIdx].className = 'lyr-word sung';
                    spans[_karaoFillIdx].style.removeProperty('--fill-pct');
                }
            }
            _karaoFillIdx = newFillIdx;
        }
    ...
    // ========== 歌词滑动拖拽（带弹簧效果） ==========
    var lyrDrag = { active: false, startY: 0, startTranslate: 0, lastY: 0, velocity: 0, lastTime: 0 };
    var lyrSpring = null; // 当前弹簧动画
    function lyrGetTranslate() {
        var inner = musicLyrScroll ? musicLyrScroll.querySelector('.music-lyric-inner') : null;
        if (!inner) return 0;
        var t = inner.style.transform;
        var m = t.match(/translateY\((.+)px\)/);
        return m ? parseFloat(m[1]) : 0;
    }
    function lyrGetBounds() {
        var inner = musicLyrScroll ? musicLyrScroll.querySelector('.music-lyric-inner') : null;
        if (!inner) return { min: -9999, max: 9999 };
        return { max: 80, min: -(inner.scrollHeight - musicLyrScroll.clientHeight) };
    }
    // 橡皮筋：超出边界时阻尼衰减
    function lyrRubberBand(val, bounds) {
        if (val > bounds.max) {
            var over = val - bounds.max;
            return bounds.max + over * 0.55; // 超出部分保留55%，明显拉得动
        } else if (val < bounds.min) {
            var over = bounds.min - val;
            return bounds.min - over * 0.55;
        }
        return val;
    }
    // 弹簧回弹动画
    function lyrSpringTo(target, onComplete) {
        var inner = musicLyrScroll ? musicLyrScroll.querySelector('.music-lyric-inner') : null;
        if (!inner) { if (onComplete) onComplete(); return; }
        if (lyrSpring) { cancelAnimationFrame(lyrSpring); lyrSpring = null; }
        var pos = lyrGetTranslate();
        var vel = 0;
        var stiffness = 0.08;  // 弹簧刚度（越小越软越弹）
        var damping = 0.82;    // 阻尼（越大振荡越久）
        function step() {
            var force = (target - pos) * stiffness;
            vel = (vel + force) * damping;
            pos += vel;
            inner.style.transform = 'translateY(' + pos + 'px)';
            if (Math.abs(target - pos) < 0.5 && Math.abs(vel) < 0.5) {
            ...
            lyrSpring = requestAnimationFrame(step);
        }
        inner.style.transition = 'none';
        step();
    }
    function lyrStartDrag(e) {
        if (!musicLyrMode) return;
        if (lyrSpring) { cancelAnimationFrame(lyrSpring); lyrSpring = null; }
        var touch = e.touches ? e.touches[0] : e;
        lyrDrag.active = true;
        lyrDrag.startY = touch.clientY;
        lyrDrag.startTranslate = lyrGetTranslate();
        lyrDrag.lastY = touch.clientY;
        lyrDrag.lastTime = Date.now();
        lyrDrag.velocity = 0;
        lyrDrag.moved = false;  // 标记是否真正拖动过
        var inner = musicLyrScroll ? musicLyrScroll.querySelector('.music-lyric-inner') : null;
        if (inner) inner.style.transition = 'none';
        // 不在这里设置 musicLyrUserScroll，等确认是拖动后再设置
        clearTimeout(musicLyrScrollTimer);
    }
    function lyrMoveDrag(e) {
        if (!lyrDrag.active) return;
        e.preventDefault();
        var touch = e.touches ? e.touches[0] : e;
        var dy = touch.clientY - lyrDrag.startY;
        if (Math.abs(dy) > 3) {
            lyrDrag.moved = true;
            musicLyrUserScroll = true;  // 确认是拖动才标记
        }
        var now = Date.now();
        var dt = now - lyrDrag.lastTime;
        if (dt > 0) lyrDrag.velocity = (touch.clientY - lyrDrag.lastY) / dt;
        lyrDrag.lastY = touch.clientY;
        lyrDrag.lastTime = now;
        var raw = lyrDrag.startTranslate + dy;
        var bounds = lyrGetBounds();
        var newT = lyrRubberBand(raw, bounds);
        var inner = musicLyrScroll.querySelector('.music-lyric-inner');
        if (inner) inner.style.transform = 'translateY(' + newT + 'px)';
    }
    function lyrEndDrag() {
        if (!lyrDrag.active) return;
        lyrDrag.active = false;
        // 没有实际拖动过 → 是点击，不处理拖动逻辑
        if (!lyrDrag.moved) return;
        var cur = lyrGetTranslate();
        var bounds = lyrGetBounds();
        var isOutOfBounds = cur > bounds.max || cur < bounds.min;
        // 惯性 + 弹簧
        if (Math.abs(lyrDrag.velocity) > 0.1 && !isOutOfBounds) {
            var target = cur + lyrDrag.velocity * 500; // 惯性更强，更容易触发边界回弹
            // 如果惯性目标超出边界，先滑到边界附近再回弹
            if (target > bounds.max || target < bounds.min) {
                var clampedTarget = Math.max(bounds.min, Math.min(bounds.max, target));
                lyrSpringTo(clampedTarget, function() {
                    clearTimeout(musicLyrScrollTimer);
                    musicLyrScrollTimer = setTimeout(function() { musicLyrUserScroll = false; }, 5000);
                });
            } else {
                // 普通惯性滑动（用 CSS transition 更流畅）
                var inner = musicLyrScroll.querySelector('.music-lyric-inner');
                if (inner) {
                    inner.style.transition = 'transform 0.5s cubic-bezier(0.15, 0.85, 0.35, 1)';
                    inner.style.transform = 'translateY(' + clampedTarget + 'px)';
                    setTimeout(function() { if (inner) inner.style.transition = ''; }, 550);
                }
                clearTimeout(musicLyrScrollTimer);
                musicLyrScrollTimer = setTimeout(function() { musicLyrUserScroll = false; }, 5000);
            }
            ...
            // 超出边界 → 弹簧回弹
            var bounceTarget = Math.max(bounds.min, Math.min(bounds.max, cur));
            lyrSpringTo(bounceTarget, function() {
                clearTimeout(musicLyrScrollTimer);
                musicLyrScrollTimer = setTimeout(function() { musicLyrUserScroll = false; }, 5000);
            });
        } else {
    ...
    if (musicLyrScroll) {
        musicLyrScroll.addEventListener('touchstart', lyrStartDrag, { passive: true });
        musicLyrScroll.addEventListener('touchmove', lyrMoveDrag, { passive: false });
        musicLyrScroll.addEventListener('touchend', lyrEndDrag);
        musicLyrScroll.addEventListener('mousedown', lyrStartDrag);
        document.addEventListener('mousemove', lyrMoveDrag);
        document.addEventListener('mouseup', lyrEndDrag);
    }
    // 「回到当前」按钮
    var lyrBackBtn = document.createElement('button');
    lyrBackBtn.className = 'lyr-back-btn';
    lyrBackBtn.textContent = '回到当前';
    lyrBackBtn.style.display = 'none';
    lyrBackBtn.addEventListener('click', function() {
        musicLyrUserScroll = false;
        clearTimeout(musicLyrScrollTimer);
        musicUpdateLyricScroll();
    });
    if (musicLyrScroll && musicLyrScroll.parentNode) {
        musicLyrScroll.parentNode.appendChild(lyrBackBtn);
    }
    // 监听 userScroll 状态显示/隐藏按钮
    var _lyrOrigUpdate = musicUpdateLyricScroll;
    musicUpdateLyricScroll = function() {
        _lyrOrigUpdate();
        lyrBackBtn.style.display = musicLyrUserScroll ? '' : 'none';
    };
    function musicUpdateLyricScroll() {
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            var isNear = (i === activeIdx - 1 || i === activeIdx + 1);
            lines[i].classList.toggle('active', isActive);
            lines[i].classList.toggle('near-active', isNear && !isActive);
            if (musicLyrWordMode && musicLyrLines[i].words && i !== activeIdx) {
                musicSetAllWordsGray(lines[i]);
            }
        }
        if (!musicLyrUserScroll && activeIdx >= 0 && lines[activeIdx] && inner) {
            var el = lines[activeIdx];
            var containerH = musicLyrScroll.clientHeight;
            var targetY = containerH * 0.35 - el.offsetTop;
            inner.style.transform = 'translateY(' + Math.min(0, targetY) + 'px)';
        }
    }
