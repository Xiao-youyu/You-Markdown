...
(function() {
    ...
    if (cmtAdminSave) cmtAdminSave.addEventListener('click', () => {
        ...
        if (pw && pw.length < 4) { cmtAdminErr.textContent = '密码至少4位'; return; }
    ...
    function cmtUpdateUI() {
        ...
        if (cmtTextarea) cmtTextarea.disabled = !loggedIn;
        if (loggedIn) {
    ...
    function cmtBindEvents() {
        ...
        cmtList.querySelectorAll('.cmt-reply-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                if (!cmtUser) { cmtCapsuleBtn && cmtCapsuleBtn.click(); return; }
