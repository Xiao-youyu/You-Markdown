...
(function() {
    ...
    btnSearch.addEventListener('click', (e) => { e.stopPropagation(); searchPanel.classList.toggle('active'); });
    ...
    btnFont.addEventListener('click', (e) => { e.stopPropagation(); fontPanel.classList.toggle('active'); });
    btnColor.addEventListener('click', (e) => { e.stopPropagation(); colorPanel.classList.toggle('active'); });
    ...
    floatTocBtn.addEventListener('click', () => tocPopup.classList.toggle('active'));
    document.addEventListener('click', (e) => { if (!tocPopup.contains(e.target) && e.target !== floatTocBtn) tocPopup.classList.remove('active'); });
    ...
    function renderCards() {
        ...
        cardsGrid.innerHTML = allFiles.map((file, i) => `
            <div class="doc-card" data-filename="${escapeHTML(file.name)}" style="animation-delay:${i*0.05}s">
                <div class="card-title">${escapeHTML(file.displayName)}</div>
                <div class="card-meta">
        ...
        sidebarFileList.innerHTML = files.map(file => `
            <div class="sidebar-item" data-filename="${escapeHTML(file.name)}">
                ...
                <div style="flex:1;min-width:0;overflow:hidden;">
                    <div class="sidebar-item-text">${escapeHTML(file.displayName)}</div>
    ...
    function renderTocItems(container, headings) {
        container.innerHTML = headings.map((h, idx) => {
            ...
            return `<div class="toc-item" data-anchor="heading-${idx}" data-level="${level}" style="padding-left:${(level-1)*16}px">${leftIcon}<div class="toc-text">${escapeHTML(removeOrdinal(h.text))}</div></div>`;
        }).join('');
        container.querySelectorAll('.toc-item').forEach(item => item.addEventListener('click', function() {
