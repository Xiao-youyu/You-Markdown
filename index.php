    ...
    if ($files) {
        ...
        foreach ($files as $file) {
            ...
            $fileList[] = [
                ...
                'hidden' => $isHidden, 'coverImage' => $coverImage
            ];
        }
    }
...
if ($action === 'read') {
    ...
    if ($meta && !empty($meta['hidden']) && !empty($meta['password_hash'])) {
        $verified = $_SESSION['verified_articles'] ?? [];
        if (!in_array($filename, $verified)) {
            echo json_encode([
                'success' => true, 'name' => $filename,
                'displayName' => $displayName,
                'hidden' => true,
                'content' => '', 'size' => filesize($filepath),
                'modified' => date('Y-m-d H:i', filemtime($filepath))
            ], JSON_UNESCAPED_UNICODE);
            exit;
        }
    }
    $content = preg_replace('/<!--META.*?-->/s', '', $content, 1);
...
if ($action === 'verify_article_password' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    header('Content-Type: application/json; charset=utf-8');
    ...
    $filepath = './data/articles/' . $filename;
    if (!file_exists($filepath) || !is_file($filepath)) {
        jsonOut(['success' => false, 'error' => '文件不存在'], 404);
    }
    $content = file_get_contents($filepath);
    $meta = null;
    if (preg_match('/<!--META(.*?)-->/s', $content, $metaMatch)) {
    ...
    if (!$meta || empty($meta['hidden']) || empty($meta['password_hash'])) {
        jsonOut(['success' => false, 'error' => '该文章未设置隐藏'], 400);
    }
    if (empty($password)) {
        jsonOut(['success' => false, 'error' => '请输入密码'], 400);
    }
    if (password_verify($password, $meta['password_hash'])) {
        $_SESSION['verified_articles'] = $_SESSION['verified_articles'] ?? [];
        if (!in_array($filename, $_SESSION['verified_articles'])) {
            $_SESSION['verified_articles'][] = $filename;
        }
        $displayName = preg_replace('/\.md$/i', '', $filename);
        if ($meta && !empty($meta['title'])) $displayName = $meta['title'];
        $pureContent = preg_replace('/<!--META.*?-->/s', '', $content, 1);
        $contentWithoutCode = preg_replace('/```[\\s\\S]*?```/', '', $pureContent);
        if ($displayName === preg_replace('/\.md$/i', '', $filename) && preg_match('/^#\\s+(.+)/m', $contentWithoutCode, $tm)) $displayName = $tm[1];
        jsonOut(['success' => true, 'name' => $filename, 'displayName' => $displayName, 'content' => $pureContent, 'size' => filesize($filepath), 'modified' => date('Y-m-d H:i', filemtime($filepath))]);
    } else {
        jsonOut(['success' => false, 'error' => '密码错误'], 401);
    }
}
if ($action === 'delete') {
...
<?php
$_ann = $_siteConfig['announcement'] ?? [];
$_annJson = htmlspecialchars(json_encode($_ann, JSON_UNESCAPED_UNICODE), ENT_QUOTES, 'UTF-8');
?>
<body<?= $_showAdminLogin ? ' data-admin-login="1"' : '' ?> data-guest-comments="<?= !empty($_siteConfig['guest_comments_enabled']) ? '1' : '0' ?>" data-bg-type="<?= htmlspecialchars($_siteConfig['bg_type'] ?? 'none') ?>" data-bg-image="<?= htmlspecialchars($_siteConfig['bg_image'] ?? '') ?>" data-bg-api-url="<?= htmlspecialchars($_siteConfig['bg_api_url'] ?? '') ?>" data-bg-api-url-mobile="<?= htmlspecialchars($_siteConfig['bg_api_url_mobile'] ?? '') ?>" data-bg-blur="<?= !empty($_siteConfig['bg_blur_enabled']) ? '1' : '0' ?>" data-bg-blur-level="<?= intval($_siteConfig['bg_blur_level'] ?? 0) ?>" data-bg-card-opacity="<?= intval($_siteConfig['bg_card_opacity'] ?? 100) ?>" data-music-playlist="<?= htmlspecialchars($_siteConfig['music_playlist_id'] ?? '3778678') ?>" data-email-verify="<?= !empty($_siteConfig['email_verify_enabled']) ? '1' : '0' ?>" data-widget-glass="<?= !empty($_siteConfig['widget_glass_enabled']) ? '1' : '0' ?>" data-card-glass="<?= !empty($_siteConfig['card_glass_enabled']) ? '1' : '0' ?>" data-card-glass-level="<?= intval($_siteConfig['card_glass_level'] ?? 16) ?>" data-announcement='<?= $_annJson ?>'>
...
<div class="dropdown-panel" id="colorPanel">
    <div class="color-panel-content">
        <div class="color-panel-header">
            <span class="color-panel-title">主题色</span>
            <button class="color-reset-btn" id="colorResetBtn">重置</button>
        </div>
        ...
        <div class="color-preview-row">
            ...
            <span class="color-preview-label" id="colorPreviewLabel">hsl(220, 60%, 50%)</span>
        </div>
    </div>
</div>
    ...
    <div class="cmt-modal-box">
        ...
        <div class="cmt-modal-body cmt-auth-slide" id="cmtAuthSlide">
            <div class="cmt-modal-form" id="cmtLoginForm">
                ...
                <div class="cmt-form-row"><div class="cmt-modal-err" id="cmtLoginErr"></div><div class="cmt-switch-inline"><span>还没有账号？</span><button class="cmt-modal-switch-link" id="cmtToRegBtn">立即注册</button></div></div>
            ...
            <div class="cmt-modal-form" id="cmtRegForm" style="display:none">
                ...
                <div class="cmt-form-row"><div class="cmt-modal-err" id="cmtRegErr"></div><div class="cmt-switch-inline"><span>已有账号？</span><button class="cmt-modal-switch-link" id="cmtToLoginBtn">去登录</button></div></div>
                <button class="cmt-modal-submit" id="cmtRegBtn">立即注册</button>
            </div>
        </div>
        <?php if (!empty($_siteConfig['oauth_enabled'])): ?>
        <div class="cmt-youyun-divider"><span>其他方式</span></div>
        <button class="cmt-youyun-btn" id="cmtYouyunBtn" data-oauth-name="<?= htmlspecialchars($_siteConfig['oauth_name'] ?? 'OAuth') ?>" data-oauth-icon="<?= htmlspecialchars($_siteConfig['oauth_icon'] ?? '') ?>">
            <?php if (!empty($_siteConfig['oauth_icon'])): ?>
                <img src="<?= htmlspecialchars($_siteConfig['oauth_icon']) ?>" width="18" height="18" style="border-radius:4px;object-fit:cover;">
            <?php else: ?>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="1.5"/><path d="M12 7v5l3 2" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>
            <?php endif; ?>
            <?= htmlspecialchars($_siteConfig['oauth_name'] ?? 'OAuth') ?> 账号登录
        </button>
        <?php endif; ?>
    </div>
</div>
...
<div class="cmt-modal-mask" id="cmtEmailVerifyModal">
    <div class="cmt-modal-box">
        <div class="cmt-modal-head"><div class="cmt-modal-title">邮箱验证</div></div>
        <div class="cmt-modal-body">
            <div class="cmt-modal-form">
                <p style="margin:0 0 12px;font-size:13px;color:var(--text-secondary);">请验证你的邮箱以完成注册</p>
                <input class="cmt-modal-input" type="email" placeholder="请输入邮箱" id="cmtVerifyEmail" style="margin-bottom:8px;">
                <div style="display:flex;gap:10px;align-items:stretch;margin-bottom:8px;">
                    <input class="cmt-modal-input" type="text" placeholder="邮箱验证码" maxlength="6" id="cmtVerifyCode" style="flex:1;margin-bottom:0;">
                    <button type="button" id="cmtVerifySendCode" style="flex-shrink:0;padding:0 16px;border:none;border-radius:var(--radius-sm);background:var(--accent);color:#fff;font-size:13px;font-weight:600;cursor:pointer;white-space:nowrap;transition:opacity .2s;height:42px;">发送验证码</button>
                </div>
                <div class="cmt-modal-err" id="cmtVerifyErr"></div>
                <button class="cmt-modal-submit" id="cmtVerifySubmit">验证并注册</button>
            </div>
        </div>
    </div>
</div>
<div class="cmt-confirm-overlay" id="cmtConfirmOverlay">
    <div class="cmt-confirm-box">
        <h3>确认操作</h3>
        <p id="cmtConfirmText">确定要执行此操作吗？</p>
...
<div class="ann-overlay" id="annOverlay">
    <div class="ann-card">
        ...
        <div class="ann-footer">
            <button class="ann-btn" id="annBtn" disabled><span id="annBtnText"></span></button>
        </div>
    </div>
</div>
...
<!-- Honeypot: invisible to users, bots will crawl -->
<a href="/trap/wp-login.php" style="position:absolute;left:-9999px;top:-9999px;opacity:0;pointer-events:none;font-size:0;" aria-hidden="true">Admin Login</a>
<a href="/trap/config.bak" style="position:absolute;left:-9999px;top:-9999px;opacity:0;pointer-events:none;font-size:0;" aria-hidden="true">config backup</a>
<a href="/trap/phpinfo.php" style="position:absolute;left:-9999px;top:-9999px;opacity:0;pointer-events:none;font-size:0;" aria-hidden="true">phpinfo</a>
<a href="/trap/.git/config" style="position:absolute;left:-9999px;top:-9999px;opacity:0;pointer-events:none;font-size:0;" aria-hidden="true">git config</a>
<a href="/trap/install" style="position:absolute;left:-9999px;top:-9999px;opacity:0;pointer-events:none;font-size:0;" aria-hidden="true">install</a>

<script>
// CSRF: fetch token + auto-inject into all POST requests
window.__csrf_token='';
fetch('api.php?action=check').then(r=>r.json()).then(d=>{
    if(d.csrf_token){
        window.__csrf_token=d.csrf_token;
